function the_grid(grid_id, url_grid,lang, per_page, page, order_id, order_direction,date_start,date_end) {
    ...
    function my_grid(id,type) { // id = id field yg di sort
        ...
        var s_field;
        var s_url = '';
        var kelas = $('#' + id).attr('class');
       
        var sort_type = ((kelas == 'sorting_asc sort' || kelas == 'sort sorting_asc' ) && type) ? 'asc' : 'desc';
        if (!kelas) {
        ...
        //alert(get_date);
        var new_class = ((kelas == 'sorting_asc sort' || kelas == 'sort sorting_asc' )) ? 'sorting' : 'sorting_asc';

        $(grid_id + ' thead tr').find('th').removeClass('sorting_asc sorting');
    ...
    //pencarian
    $(grid_id +' .cari').keypress(function(e) {
        if (e.which == 13) {
            var val;
            var key;
            var kelas;
    ...
    /*$('.search_date').click(function(e) {
        if (e.which == 13) {
            ...
            var id;
            var url = '';
            $(grid_id).find('.cari_date').each(function() {
                val = $(this).val();
                key = $(this).attr('id');
                url += '&' + key + '=' + val;
            })
        ...
        $(grid_id + ' thead tr th').each(function() {
            if ($(this).attr('title') == 'Sort') {
                ...
                //$(this).find('span').each(function() {
                    ...
                    //console.log(kelas);
                    if ((kelas == 'sorting_asc sort' || kelas == 'sorting sort') || (kelas == 'sort sorting_asc' || kelas == 'sort sorting')) {
                        hasil = ret;
                    }
                //})
            }
    ...
    function kolom_type() {
        ...
        $(grid_id + ' thead tr th').each(function() {
            if ($(this).attr('title') == 'Sort') {
                ...
                //$(this).find('span').each(function() {
                    kelas = $(this).attr('class');
                    if ((kelas == 'sorting sort') || (kelas == 'sort sorting') ) {
                        hasil = 'desc';
                    }
                    else if ((kelas == 'sorting_asc sort') || (kelas == 'sort sorting_asc'))  {
                        hasil = 'asc';
                    }
                //})
            }
    ...
    $(grid_id + ' .reload').click(function() {
        ...
        $(grid_id).find('input:text').val('');
        $(grid_id).find('select').val("0");
        date = false;
        my_grid(order_id);
    })
