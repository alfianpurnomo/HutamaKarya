    ...
    public function list_data()
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            foreach ($records as $row => $record) {
                ...
                $return['data'][$row]['is_default'] = ($record['is_default'] == 1) ? 'Default' : 'Not';
            }
            json_exit($return);
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                $data_log = [
                    ...
                    'action'   => 'Update Site',
                    'desc'     => 'Update Site; ID: '.$id.'; Data: '.json_encode($post),
                ];
            ...
            if (isset($post['id']) && $post['id'] > 0 && ctype_digit($post['id'])) {
                ...
                if ($detail && ($detail['site_logo'] != '' && file_exists(UPLOAD_DIR. $this->class_path_name. '/'.$detail['site_logo']))) {
                    ...
                    $data_log = [
                        'id_user'  => id_auth_user(),
                        'id_group' => id_auth_group(),
                        'action'   => 'Delete Picture Site Setting',
                        'desc'     => 'Delete Picture Site Setting; ID: '.$id.'; Data: '.json_encode($post),
                    ];
