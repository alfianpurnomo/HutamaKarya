...
class Menu extends CI_Controller
{
    ...
    public function add()
    {
        ...
        $this->data['form_action_function']  = site_url($this->class_path_name.'/add_function');
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                $listFunction = $post['listFunction'];
                unset($post['listFunction']);
                // insert data
                $id = $this->Menu_model->InsertRecord($post);
                $post['listFunction'] = $listFunction;
                ...
                if($id){
                    ...
                    if($listFunction){
                        ...
                        if($dataFunction){
                            $this->db->insert_batch('auth_menu_function',$dataFunction);  
                        }
                    }
    ...
    public function edit($id = 0)
    {
        ...
        $menu_data                    = $this->Menu_model->MenusData();
        #echo $record['parent_auth_menu'];
        $this->data['auth_menu_html'] = $this->Menu_model->PrintAuthMenu($menu_data, '',$record['parent_auth_menu']);#$this->Menu_model->PrintAuthMenu($menu_data, '', $record['parent_auth_menu'], $disabled_menu);
        /*debugvar($disabled_menu);
        die();*/
        if ($this->input->post()) {
            $post = $this->input->post();
            // debugvar($post);
            ...
            if ($this->validateForm($id)) {
                ...
                $post['file']          = strtolower($post['file']);
                $listFunction = $post['listFunction'];
                $listDeleteFunction = $post['listDeleteFunction'];
                unset($post['listFunction']);
                unset($post['listDeleteFunction']);
                
                
                // update data
                $this->Menu_model->UpdateRecord($id, $post);
                $post['listFunction']       = $listFunction;
                $post['listDeleteFunction'] = $listDeleteFunction;
                ...
                $dataDeleteFunction = [];
                if($listDeleteFunction){
                    foreach ($listDeleteFunction as $key => $value) {
                        $dataDeleteFunction[] = $value['id_auth_menu_function'];
                    }  
                    if($dataDeleteFunction){
                        //echo 'asd';
                        $this->Menu_model->DeleteFunctionWhereIn('auth_menu_function',$dataDeleteFunction); 
                        $this->Menu_model->DeleteFunctionWhereIn('auth_function_group',$dataDeleteFunction);  
                    }
                }
                // insert to log
                $data_log = [
