    ...
    private function generateTabelRow($employeeid,$startDate,$endDate,$listDate,$rangeDate=7){
        ...
        try{
            if($rangeDate){
                if($rangeDate < 8){
                    ...
                    if($getData){
                        ...
                        foreach ($getData as $key => $value) {
                            ...
                            $result[$keyid]['notes'] = $value['notes'];
                        ...
                        foreach ($result as $key => $value) {
                            ...
                            foreach ($listDate as $x => $y) {
                                ...
                                $attr = '';
                                if($hours){
                                    $attr = 'class="editTasks" data-title="'.$value['title'].'" data-activity_name="'.$value['activity_name'].'" data-employeeid="'.$value['employeeid'].'" data-id_project="'.$value['projectid'].'" data-id_activity="'.$value['activityid'].'" data-notes="'.$value['notes'].'" data-date="'.$y['date'].'" data-hours="'.$hours.'"';
                                }
                        ...
                        $return['status'] = alert_box('List tasks not found for this user', 'info');
                        $return['html'] = '';
                    }
                }else{
                ...
                $return['html'] = '';
                $return['status'] = alert_box('Choose date range first', 'info');
                ...
                if($rangeDate < 8){
                    ...
                    if($getDataEmployee){
                        ...
                        foreach ($getDataEmployee as $key => $value) {
                            ...
                            $result[$keyid]['employee_name']        = $value['firstname'].' '.$value['lastname'];
                            $result[$keyid]['activity_name']        = $value['activity_name'];
                            $result[$keyid]['employeeid']           = $value['employeeid'];
                            $result[$keyid]['projectid']            = $value['projectid'];
                            $result[$keyid]['activityid']           = $value['activityid'];
                        ...
                        foreach ($result as $key => $value) {
                            ...
                            foreach ($listDate as $x => $y) {
                                ...
                                if($hours){
                                    ...
                                    $attr = 'class="'.$edit.'" data-title="'.$value['title'].'" data-employee_name="'.$value['employee_name'].'" data-activity_name="'.$value['activity_name'].'" data-employeeid="'.$value['employeeid'].'" data-id_project="'.$value['projectid'].'" data-id_activity="'.$value['activityid'].'" data-notes="'.$value['notes'].'" data-date="'.$y['date'].'" data-role="'.$value['employee_role'].'" data-hours="'.$hours.'"';
                                }
                                $html .= '<td ><div '.$attr.'>'.$hours.'</div></td>';# code...
                            }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if(is_superadmin() && $post['view']==2){
                ...
                if($employeeid){
                    ...
                    $return['status'] = alert_box('Choose Employee first','warning');
                }
            }else{
                $employeeid = employeeid();
                $listTeamLead = $this->TM->getProjectTeamLead($employeeid);
                
                if($listTeamLead && $post['view']==1){
                    //debugvar($listTeamLead);
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            if($this->validateFormAjax()){
                ...
                $check = $this->db
                           ...
                           ->row_array();
                #echo $this->db->last_query();
                $total_hours = ($check['hours']-$post['hours_before']) + $post['hours_edit'];
                #json_exit($check);
                if($total_hours > 12){
                    $return['raw_data'] = [];
                    $return['html'] = '';
