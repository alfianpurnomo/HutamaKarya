...
class Department extends CI_Controller
{
    ...
    public function list_data()
    {
        ...
        if ($this->input->post() && $this->input->is_ajax_request()) {
            ...
            $count_all_records         = $this->Department_model->CountAllData();
            $count_filtered_records    = $this->Department_model->CountAllData($param);
            $records                   = $this->Department_model->GetAllData($param);
            $return                    = [];
            ...
            foreach ($records as $row => $record) {
                ...
                $return['data'][$row]['department_name']          = $record['department_name'];
                $return['data'][$row]['division_name']          = $record['division_name'];
                $return['data'][$row]['head_of_department_name']  = $record['head_of_department_name'];
    ...
    public function add()
    {
        $this->data['master_employee']    = $this->Department_model->GetEmployee();
        $this->data['master_division']    = $this->Department_model->GetDivision();
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm()) {
                ...
                // update data
                $id = $this->Department_model->InsertRecord($post);
                ...
                $data_log = [
                    ...
                    'desc'     => 'Add Department; ID: '.$id.'; Data: '.json_encode($post),
                ];
    ...
    public function edit($id = 0)
    {
        ...
        $record = $this->Department_model->GetDepartment($id);
        ...
        if ($this->input->post()) {
            ...
            if ($this->validateForm($id)) {
                ...
                $post['id_auth_user']   = id_auth_user();
                $this->Department_model->UpdateRecord($id, $post);
                ...
                $data_log = [
                    ...
                    'desc'     => 'Edit Department; ID: '.$id.'; Data: '.json_encode($post),
                ];
                insert_to_log($data_log);
                // end insert to log
                $this->session->set_flashdata('flash_message', alert_box('Success.', 'success'));
                            ...
                            if ($id == id_auth_user()) {
                                ...
                                if (is_superadmin()) {
                                    ...
                                    $data_log = [
                                        ...
                                        'desc'     => 'Delete User Department; ID: '.$id.';',
                                    ];
                                    insert_to_log($data_log);
                                    // end insert to log
                                } else {
                                    $json['error'] = alert_box('You don\'t have permission to delete this record(s). Please contact the Departmentistrator.', 'danger');
                                    break;
                                }
                            }
                        } else {
    ...
    private function validateForm($id = 0)
    {
        ...
        $rules = [
            [
                ...
                'rules' => 'required|min_length[3]|max_length[32]|callback_check_department_name_exists['.$id.']',
            ]
        ];
    ...
    {
        if (!$this->Department_model->checkExistsDepartmentName($string, $id)) {
